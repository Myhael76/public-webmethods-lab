...
if [ $? -eq 0 ]; then
	...
	if [ "${RESULT_setupLocal}" -eq 0 ]; then
		...
		cp -r "${WMLAB_INSTALL_HOME}/IntegrationServer/packages/WmJDBCAdapter"            "${DOCKER_CONTEXT_FOLDER}/SAG_HOME/IntegrationServer/packages"
		cp -r "${WMLAB_INSTALL_HOME}/IntegrationServer/packages/WmART"                    "${DOCKER_CONTEXT_FOLDER}/SAG_HOME/IntegrationServer/packages"
		...
		logI "Downloading sqlite driver"
		downloadCmd="curl -o "'"${DOCKER_CONTEXT_FOLDER}/SAG_HOME/IntegrationServer/packages/WmJDBCAdapter/code/jars/${WMLAB_JDBC_DRIVER_FILENAME}" "${WMLAB_JDBC_DRIVER_URL}"'
		...
		logI "Building docker image for msr-lean-sqlite-devel-${WMLAB_PRODUCTS_VERSION}"

		pushd . 
		cd "${DOCKER_CONTEXT_FOLDER}"/
		controlledExec "docker build -t msr-lean-sqlite-devel-${WMLAB_PRODUCTS_VERSION} ." "05.buildContainer"
