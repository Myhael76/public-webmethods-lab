...
onInterrupt(){
    ...
    cd "${WMLAB_INSTALL_HOME}/UniversalMessaging/server/${WMLAB_REALM_NAME}/bin"
...
if [ ! -d ${WMLAB_INSTALL_HOME}/UniversalMessaging/server/${WMLAB_REALM_NAME}/bin ]; then
    logI "Realm ${WMLAB_REALM_NAME} does not exist, creating now"
    cd ${WMLAB_INSTALL_HOME}/UniversalMessaging/tools/InstanceManager/
